 ...
 *
 *Side project based off the solar simulation lesson
 * the project generally turn the led light on when it gets dark or its room level
 * if its bright turn off
 */
...
void loop() {
  ...
  if (light_on && current_intensity > HIGH_INTENSITY_LIMIT) {
    ...
    digitalWrite(LIGHT, ON);  // Light is on, turn it off
  ...
  Serial.print(", ");
  Serial.print(current_intensity);  // show current intensity in percent
  Serial.print(", ");
  Serial.println(map(current_intensity, 0, 1023, 0, 100));  
